...
void MatchDocuments(const SearchServer& search_server, const std::string& query) {
    ...
    try {
        ...
        for (int index = 0; index < document_count; ++index) {
            const int document_id = *search_server.GetDocIdByIndex(index);
...
std::vector<int>::const_iterator SearchServer::GetDocIdByIndex(int x) const
{
    return document_ids_.begin()+x;
}

const std::map<std::string, double>& SearchServer::GetWordFrequencies(int document_id) const
{
    static const std::map<std::string, double> emptymap;
